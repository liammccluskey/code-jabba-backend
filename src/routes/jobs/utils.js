...
const generateMongoFilterFromJobFilters = ({
    ...
    const filter = {
        $and: [
            ...
            includedLanguages.length && excludedLanguages.length ? { languages: { $in: includedLanguages, $nin: excludedLanguages } } : {},
            includedLanguages.length && !excludedLanguages.length ? { languages: { $in: includedLanguages } } : {},
            !includedLanguages.length && excludedLanguages.length ? { languages: { $nin: excludedLanguages } } : {},
            includedSkills.length && excludedSkills.length ? { skills: { $in: includedSkills, $nin: excludedSkills } } : {},
            ...
            [0, '0'].includes(salaryMin) ? {} : {salaryType: { $ne: 'not-provided' }},
